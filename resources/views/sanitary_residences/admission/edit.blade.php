...
<form method="POST" class="form-horizontal" action=" {{ route('sanitary_residences.admission.update', $admission) }} ">
    ...
    <div class="form-row">
        <fieldset class="form-group col-4 col-md-4">
            ...
            <input type="number" class="form-control" name="people" id="for_people" required placeholder="" autocomplete="off" value="{{$admission->people}}">
        </fieldset>
        <fieldset class="form-group col-4 col-md-4">
            <label for="for_name">¿Con cuantas habitaciones cuenta el hogar?</label>
            <input type="number" class="form-control" name="rooms" id="for_rooms" required placeholder="" autocomplete="off" value="{{$admission->rooms}}">
        </fieldset>
        <fieldset class="form-group col-4 col-md-4">
    ...
    <div class="form-check">
        ...
        <input class="form-check-input" type="radio" name="water" id="exampleRadios1" value="1" readonly required {{ ($admission->water=='1')?'checked':'' }}>
    ...
    <div class="form-check">
        <input class="form-check-input" type="radio" name="water" id="exampleRadios2" value="0" readonly required {{ ($admission->water=='0')?'checked':'' }}>
    ...
    <div class="form-check">
        <input class="form-check-input" type="radio" name="isolate" id="exampleRadios2" value="0" {{ ($admission->isolate=='0')?'checked':'' }}>
        <label class="form-check-label" for="exampleRadios2">
            NO
        </label>
    </div>
    ...
    <tbody>
        <tr>
            ...
            <td class="text-center"><input name="respiratory" id="for_respiratory" type="radio" value="1" {{ ($admission->respiratory=='1')?'checked':'' }}></td>
            <td class="text-center"><input name="respiratory" id="for_respiratory" type="radio" value="0" {{ ($admission->respiratory=='0')?'checked':'' }}></td>
        </tr>
        <tr>
            <td>¿Tiene alguna dificultad para realizar sus actividades básicas (o diarias) sin ningún tipo de ayuda?</td>
            <td class="text-center"><input name="basicactivities" id="for_basicactivities" type="radio" value="1" {{ ($admission->basicactivities=='1')?'checked':'' }}></td>
            <td class="text-center"><input name="basicactivities" id="for_basicactivities" type="radio" value="0" {{ ($admission->basicactivities=='0')?'checked':'' }}></td>
        </tr>
        <tr>
            <td>¿Actualmente se encuentra o se ha encontrado en tratamiento por alguna adicción (como alcohol, drogas o tabaco)?</td>
            <td class="text-center"><input name="drugs" id="for_drugs" type="radio" value="1" {{ ($admission->drugs=='1')?'checked':'' }}></td>
            <td class="text-center"><input name="drugs" id="for_drugs" type="radio" value="0" {{ ($admission->drugs=='0')?'checked':'' }}></td>
        </tr>
        <tr>
            <td>¿Tiene enfermedades crónicas no compensadas (ej: hipertensión, diabetes)?</td>
            <td class="text-center"><input name="chronic" id="for_chronic" type="radio" value="1" {{ ($admission->chronic=='1')?'checked':'' }}></td>
            <td class="text-center"><input name="chronic" id="for_chronic" type="radio" value="0" {{ ($admission->chronic=='0')?'checked':'' }}></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="text-center"><input name="old" type="radio" value="1" {{ ($admission->old=='1')?'checked':'' }}></td>
            <td class="text-center"><input name="old" type="radio" value="0" {{ ($admission->old=='0')?'checked':'' }}></td>
        </tr>
