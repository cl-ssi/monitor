...
<form method="POST" class="form-horizontal" action="{{ route('sanitary_residences.admission.store') }}">
    ...
    <div class="form-row">
        ...
        <fieldset class="form-group col-4 col-md-4">
            ...
            <input type="text" class="form-control" name="name" id="for_name" required placeholder="" autocomplete="off" @if($patient->suspectCases) value="{{ ($patient->suspectCases->where('pscr_sars_cov_2', 'positive')->last())? $patient->suspectCases->where('pscr_sars_cov_2', 'positive')->last()->pscr_sars_cov_2_at->format('d-m-Y H:i'):''  }}" @endif readonly>
        </fieldset>
